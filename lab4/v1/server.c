...
// function to send file content to client
void send_file_info_to_client(){
   ...
   struct sigaction sa;
	...
	times.it_value.tv_usec = timeout*1000;
	...
	continue;
	// will go at the starting of the loop, use reset values as per signal handler
	}
	
	cur_seq= cur_seq%max_seq_numbr; // setting next start sequence
...
void wait_ack(int sig){
	...
	fseek(fp,pos_copy,0); // reset the file to previous position
	cur_seq = prev_cur_seq; // reset the sequence number
	cur_max_win = prev_max_win_size; // reset current the max window limit
...
int main(int argc, char *argv[]){
	...
	if(argc!=7){
		printf("%s\n","please provide all 7 command line arguments as follows - ");
		printf("%rrunners server-IP server-port secret-key blocksize windowsize \n");
		printf("******************************\n");
		return 0;
	}
    ...
    while(1){
	 ...
	 if(res==-1){
	  ...
	  continue;
    	...
    	end_of_the_file =0;
    	if(frk==0){
    		send_file_info_to_client(); // calling the function for sending file
