	...
	while(end_of_the_file==0){
		...
		while(cur_seq<cur_max_win){
	        ...
	        for(int k=0;k<read_return;k++){
	        	unsigned int x = (int)server_message[k];
	        	unsigned int ss=x;
	        	x = bbencode(x, pub_key);
	        	buffer[u]=(unsigned char)((char)x);
	        	last_pack[u]=(unsigned char)((char)x);
	        	u++;
	        }
	        ...
	        if (feof(fp)){
	        	printf("sending last %x\n",last_pack[block_size+1] );
	        	end_of_the_file=1;
...
int main(int argc, char *argv[]){
	...
	if(argc!=6){
		printf("%s\n","please provide all 6 command line arguments as follows - ");
