	...
	Functions:
		...
		e. main() -> creates socket, accepts requests from client, read request, do ip address check, 
				...
				Then it checks the file name.
			       ...
			       While sending each byte, it first gets the integer corresponding to the character, then passes to encode function which returns another unsigned int
			       which is then converted to unsigned char. Thus the final message is formed, this is passed to client.
...
2. client.c
	...
	Functions:
		...
		i. main() -> will prepare the message, send request to server.
			     Client will first convert the received unsigned char to char , then to its corresponding integer, then applies bbdecode() function to get the
			     actual integer value, then convert it back to corresponding char which is the real message.
		            store the response file in "Client" directory within v3 folder, Calculates throughput.
				


		Point to note : rrunner will store the response file within Client directory in v3 folder.
