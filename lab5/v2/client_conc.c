...
float update_lambda(float lambda){
	...
	if(method==0){
		...
		// E
		new_lambda = lambda + (epsilon * ((float)target_buf - size_till_now)/(float)block_size) - (beta*(lambda - gamma_param)*(float)(4096/(float)block_size)) + delta*block_size;
	}
